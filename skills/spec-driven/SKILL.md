...
reimplement spec-driven development — it **bootstraps** OpenSpec into the repo and **wires** it to the
...
feature, ask the user: *apply OpenSpec (spec-driven) for this one?* **Only if they say yes** do we run the
OpenSpec flow. If they decline, proceed with the normal rules skills (`fe-coding` / `ticket-workflow`) —
...
3. **CLI reachable** — `npx openspec@latest --version` (first run downloads it). If offline / npm is
   unreachable, report that plainly and offer to proceed without spec-driven mode this session.
...
via `/project-setup` so the SessionStart hook knows this repo is spec-driven.

## STEP 2 — Per-ticket gate

Once `openspec/` exists, the SessionStart hook reminds Claude that this repo *can* run spec-driven. But
...
`fe-coding`. Offer it when the work is a **new capability or a behaviour change**: a new domain/screen, a
...
| Apply | `/opsx:apply` | Implement against `tasks.md`. Code is written by **`fe-coding`** under the house rules. Tick tasks as they land. |
...
- **`tasks.md` phases align with `fe-coding`**: Foundation (models + `apiClient[Domain]`) → Core UI
...
- **Unify the change id with the ticket tracker.** When work comes from a ticket, name the change after
  it: `/opsx:propose tlm-1234-<slug>`. This replaces `ticket-workflow`'s ad-hoc plan file — the OpenSpec
  change **is** the plan. (Config `tlm.tickets.planDir` becomes moot once spec-driven is on.)
- **`fe-coding` reads the active change.** While a change is un-archived and matches the file being
...
| Missing | Behaviour |
|---------|-----------|
| Node < 20.19 | Report it; OpenSpec CLI won't run. Fall back to ordinary `fe-coding`. |
| npm/network unreachable | Report it; proceed under `fe-coding` without OpenSpec this session. |

See `setup/tlm-config.reference.json` → `tlm.specDriven` and `skillRequirements.spec-driven` for the
config contract. Never treat a missing OpenSpec as a hard stop — unlike `figma-to-code`, this skill
degrades to ordinary `fe-coding`.
