...
> [`shared/03-component-patterns.md`](./03-component-patterns.md) for the component hierarchy.
...
1. **Read the knowledge base** in this `ai/` folder to understand:
   - Project architecture & the framework-agnostic `_modules/` philosophy (`shared/01`)
   - Styling / UI conventions — Tailwind CSS (`shared/02`)
   - Component patterns & hierarchy (`shared/03`)
   - TypeScript enums & constants (`shared/04`)
   - Validation patterns (`shared/05`)
...
- [ ] Loading/empty states expressed via props (no mount/unmount branching)
