...
- **Thin routing layer; logic in Screen components** — `ai/shared-fe/01`
- **Component hierarchy**: Basic → Base → Common → Domain → Screen — `ai/shared-fe/03`
- **Function minimalism (YAGNI)**: inline handlers + `TODO`, no premature `useCallback`/`useMemo` — `ai/shared-fe/03`
- **Never use raw primitives** for layout/text — use `Col`/`Row` + a text component — `ai/shared-fe/03`
- **Domain-implicit API clients**: `apiClient[Domain]` exposing `useQuery[Entity]` /
  `useMutationCreate/Update/Delete` — `ai/shared-fe/07`, `ai/nextjs/*/03`, `ai/reactnative/04`
- **Server state via TanStack Query**; invalidate related queries on mutation success
- **Forms via React Hook Form + Zod**, schema-first, no manual/state validation — `ai/shared-fe/05`
- **Enums / const objects over string literals** (`E`-prefixed) — `ai/shared-fe/04`
...
| Styling | Tailwind CSS (`shared-fe/02`) | `StyleSheet.create` + theme constants (`reactnative/02`) |
...
- Shared conventions apply on both — keep `ai/shared-fe/` as the base for either target.
