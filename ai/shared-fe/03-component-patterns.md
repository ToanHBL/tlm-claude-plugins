...
**Why this is a MUST, not a style preference:** a hidden block is invisible to QC. A tester or reviewer
looking at the screen cannot tell whether the section is *empty by design* or *silently broken* — there
is nothing on screen to verify against. An explicit empty state makes "no data" a testable state.

- **MUST** keep the section header rendered and show an empty-state component in place of the list.
- **MUST NOT** wrap an entire section in `data.length > 0 ? (…) : null`. Only the list-vs-empty
  *content* toggles; the header stays.
- Prefer a wrapper component (`ViewEmptyFallBack` / `BaseEmptyFallBack`) over an inline
  `isEmpty ? <Empty/> : children` ternary — it keeps the JSX flat and reads as one declarative block.
  This is the same principle as the Loading States rule above: express state through **props/wrappers**,
  not through `if` branches that mount and unmount whole subtrees.
- Applies to any data-driven section: lists, digests, search results, notifications, dashboards.

```tsx
// ❌ Whole section disappears when empty — QC cannot see or test it
{attention.length > 0 ? (
  <Col className="gap-2">
    <SectionHead title="Needs attention" />
    {attention.map((i) => <DigestItemRow key={i.id} item={i} />)}
  </Col>
) : null}

// ❌ The empty check leaks an if/else ternary into the middle of the JSX
<Col className="gap-2">
  <SectionHead title="Needs attention" />
  {attention.length === 0 ? (
    <BaseEmptyState title={t('allClear')} message={t('nothingNeedsAttention')} />
  ) : (
    attention.map((i) => <DigestItemRow key={i.id} item={i} />)
  )}
</Col>

// ✅ Header always renders; the wrapper owns the empty case
<Col className="gap-2">
  <SectionHead title="Needs attention" />
  <BaseEmptyFallBack
    isEmpty={attention.length === 0}
    title={t('allClear')}
    message={t('nothingNeedsAttention')}
  >
    <Col className="gap-1">
      {attention.map((i) => <DigestItemRow key={i.id} item={i} />)}
    </Col>
...
For a chain that also handles loading and error, put those branches first and let the empty wrapper own
only the final empty-vs-content step:

```tsx
{query.isLoading && !query.data ? (
  <BaseSkeletonList count={5} />
) : query.isError ? (
  <BaseErrorState onRetry={query.refetch} />
) : (
  <BaseEmptyFallBack isEmpty={(query.data ?? []).length === 0} title={t('noRegions')} message={t('…')}>
    {(query.data ?? []).map((r) => <RegionCard key={r.id} region={r} />)}
  </BaseEmptyFallBack>
)}
```

**React Native:** a `FlatList`'s `ListEmptyComponent={<BaseEmptyState … />}` already satisfies this rule
(it is not an `if` branch in the tree) — leave it as-is.

**Exception:** a purely decorative element with no header and no QC-relevant state (e.g. an optional
badge) may still be omitted when absent.
