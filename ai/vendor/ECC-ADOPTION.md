...
| `agents/architect.md` | `fe-coding` STEP 1.5 / 1.6 | A generic architecture-review prompt with no cross-repo or domain-boundary content. Not a substitute for the ecosystem rules. |
...
| `rules/git-workflow.md` | Prescribes a commit-message format. Ours comes from `ticket-workflow` and the ticket tracker; two formats is worse than either. |
...
`09`–`13` are `ai/` reference files loaded on demand while `fe-coding` STEP 1.6 is inline in the skill:
